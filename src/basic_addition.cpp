...
int main() {
  ...
  const uint8_t *metallib_data = getsectiondata(&_mh_execute_header, "__TEXT",
                                                "__metallib", &metallib_size);
  ...
  auto addFunctionPSO =
      device->newComputePipelineState(addFunction.get(), &error);
  ...
  auto bufferA = NS::TransferPtr(
      device->newBuffer(kBufferSize, MTL::ResourceStorageModeShared));
  auto bufferB = NS::TransferPtr(
      device->newBuffer(kBufferSize, MTL::ResourceStorageModeShared));
  auto bufferResult = NS::TransferPtr(
      device->newBuffer(kBufferSize, MTL::ResourceStorageModeShared));
  ...
  // Note that we don't need to call TransferPtr here as this is an
  ...
  NS::UInteger threadGroupSize =
      addFunctionPSO->maxTotalThreadsPerThreadgroup();
